...
/* harmony default export */ var transfer_panelvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    loadRenderData: function loadRenderData(reset) {
      var _this7 = this;

      var lazy = Math.max(this.lazy, 0);
      if (lazy) {
        var loadNext = function loadNext() {
          var _renderData;

          var to = lazy * _this7.lazyPage;
          var from = lazy * (_this7.lazyPage - 1);
          var len = _this7.filteredData.length;
          if (to >= len) {
            _this7.lazyLoaded = true;
            to = len;
          }
          (_renderData = _this7.renderData).push.apply(_renderData, _this7.filteredData.slice(from, to));
          _this7.lazyPage++;
        };
        if (reset) {
          this.lazyPage = 1;
    ...
    doScroll: function doScroll(e) {
      ...
      if (this.lazy && !this.lazyLoaded) {
        var isApproachBottom = target.scrollTop + target.offsetHeight + 50 >= target.scrollHeight;
        if (isApproachBottom) {
          this.loadRenderData();
        }
      }

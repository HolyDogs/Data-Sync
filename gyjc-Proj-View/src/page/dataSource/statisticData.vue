<template>
  <div class="app-container page_02" align="center" style="height:calc(100vh - 64px)">
    ...
    <div class="filter-container">
       <el-row>
          ...
          <el-select v-model="listQuery.zbfl" style="width: 180px" filterable clearable placeholder="请选择">
            <el-option
  ...
  <!--       <el-table-column width="180px" align="center" label="绑定指标来源表">
   ...
   <dataCount dataType="STATISTIC_ZB" />
    </div>
...
export default {
  ...
  methods: {
    getzbflList() {
      let me = this;
      this.$axios.get('statistic/getZbflList',{}, function(r){
        me.zbflList = r.data;
      });
    },
    async getList(pageNum, zbmc, zbfl,startTime,endTime) {
      let me = this;
      //设置加载动画
      this.listLoading = true;
      this.$axios.get('statistic/getList',{pageNum: pageNum, zbmc: zbmc, zbfl: zbfl,startTime:startTime,endTime:endTime}, function(r){
        //页数
    ...
    handleFilter() {
      ...
      //查询
      this.getList(this.pageParam.currentPage, this.listQuery.zbmc, this.listQuery.zbfl,this.listQuery.startTime, this.listQuery.endTime);
    },
    changePage() {
      //查询
      this.getList(this.pageParam.currentPage, this.listQuery.zbmc, this.listQuery.zbfl,this.listQuery.startTime, this.listQuery.endTime);
